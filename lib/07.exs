...
defmodule Bag do
  def parse(string) do
    [_, name, contains] = Regex.run(~r/^(.+) bags contain (.+)\.$/, string)

    contains = 
      if contains == "no other bags" do
        %{}
      else
        contains
        |> String.split(", ")
        |> Enum.reduce(%{}, fn bag, acc ->
          [_, num, name] = Regex.run(~r/^(\d+) (.+) bags?$/, bag)
          Map.put(acc, name, num |> String.to_integer)
        end)
      end
  ...
  def contains?(bags, target, bag) do
    Map.has_key?(bag, target) or bag |> Enum.any?(fn {inner, _} -> contains?(bags, target, Map.get(bags, inner)) end)
  end

  def count(bags, bag) do
    bag
    |> Enum.map(fn {name, num} -> num * (count(bags, Map.get(bags, name)) + 1) end)
    |> Enum.sum
  end
end
