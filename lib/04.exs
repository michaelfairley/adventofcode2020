...
defmodule Day04 do
  def valid?(passport)do
    pairs = passport |> String.split(" ") |> Enum.map(& &1 |> String.split(":"))
    
    keys = pairs |> Enum.map(& &1 |> List.first) |> MapSet.new

    desired_keys = MapSet.new(["hcl", "iyr", "eyr", "ecl", "pid", "byr", "hgt"])
    MapSet.subset?(desired_keys, keys) and Enum.all?(pairs, &valid_field?/1)
  end

  def valid_field?([key, value]) do
    case key do
      "cid" ->
        true
      "pid" ->
      ...
      "hgt" ->
        case Regex.run(~r/(\d+)(in|cm)/, value) do
          [_, num, unit] ->
            ...
            case unit do
              "in" -> num >= 59 and num <= 76
              "cm" -> num >= 150 and num <= 193
            end
          _ -> false
        end
      "ecl" ->
...
assert Day04.valid?("pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980 hcl:#623a2f")
assert Day04.valid?("eyr:2029 ecl:blu cid:129 byr:1989 iyr:2014 pid:896056539 hcl:#a97842 hgt:165cm")
assert Day04.valid?("hcl:#888785 hgt:164cm byr:2001 iyr:2015 cid:88 pid:545766238 ecl:hzl eyr:2022")
assert Day04.valid?("iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719")

assert !Day04.valid?("eyr:1972 cid:100 hcl:#18171d ecl:amb hgt:170 pid:186cm iyr:2018 byr:1926")
assert !Day04.valid?("iyr:2019 hcl:#602927 eyr:1967 hgt:170cm ecl:grn pid:012533040 byr:1946")
assert !Day04.valid?("hcl:dab227 iyr:2012 ecl:brn hgt:182cm pid:021572410 eyr:2020 byr:1992 cid:277")
assert !Day04.valid?("hgt:59cm ecl:zzz eyr:2038 hcl:74454a iyr:2023 pid:3556412378 byr:2007")

passports
|> Enum.count(&Day04.valid?/1)
|> IO.inspect
