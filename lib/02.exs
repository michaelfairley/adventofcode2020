...
defmodule Day02 do
  def is_valid(line) do
    regex = ~r/(?<min>\d+)-(?<max>\d+) (?<target>.): (?<string>.+)/
    captures = Regex.named_captures(regex, line)

    target = captures["target"]
    
    min = captures["min"] |> String.to_integer
    max = captures["max"] |> String.to_integer

    actual = captures["string"] |> String.graphemes |> Enum.count(fn(c) -> c == target end)

    actual >= min && actual <= max
  end
  
  def is_valid2(line) do
    regex = ~r/(?<a>\d+)-(?<b>\d+) (?<target>.): (?<string>.+)/
    captures = Regex.named_captures(regex, line)

    target = captures["target"]
    
    a = captures["a"] |> String.to_integer |> Kernel.-(1)
    b = captures["b"] |> String.to_integer |> Kernel.-(1)
...
assert Day02.is_valid("1-3 a: abcde")
assert !Day02.is_valid("1-3 b: cdefg")
assert Day02.is_valid("2-9 c: ccccccccc")

assert Day02.is_valid2("1-3 a: abcde")
assert !Day02.is_valid2("1-3 b: cdefg")
assert !Day02.is_valid2("2-9 c: ccccccccc")

input
|> String.trim
|> String.split("\n")
|> Enum.count(&Day02.is_valid2/1)
|> IO.inspect
