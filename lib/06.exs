...
groups = input |> String.trim |> String.split("\n\n")

groups
|> Stream.map(fn group ->
  group
  |> String.split("\n")
  |> Stream.map(& &1 |> String.codepoints |> MapSet.new)
end)
|> Stream.map(fn sets -> sets |> Enum.reduce(&MapSet.intersection/2) |> MapSet.size end) # use union for part 1
|> Enum.sum
|> IO.inspect
